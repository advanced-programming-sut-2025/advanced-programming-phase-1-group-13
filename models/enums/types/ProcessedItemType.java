...
public enum ProcessedItemType implements ItemType {
    ...
    CHEESE("Cheese", "It's your basic cheese.", 100, 3,
            createIngredientsMap(new AnimalProduct(AnimalProductType.COW_MILK), 1), 230),
    LARGE_CHEESE("Large Cheese", "It's your basic cheese.", 100, 3,
            createIngredientsMap(new AnimalProduct(AnimalProductType.LARGE_COW_MILK), 1), 345),
    GOAT_CHEESE("Goat Cheese", "Soft cheese made from goat's milk.", 100, 3,
            createIngredientsMap(new AnimalProduct(AnimalProductType.GOAT_MILK), 1), 400),
    LARGE_GOAT_CHEESE("Large Goat Cheese", "Soft cheese made from goat's milk.", 100, 3,
            createIngredientsMap(new AnimalProduct(AnimalProductType.LARGE_GOAT_MILK), 1), 600),
    BEER("Beer", "Drink in moderation.", 50, 24,
            createIngredientsMap(new Crop(CropType.WHEAT), 1), 200),
    VINEGAR("Vinegar", "An aged fermented liquid used in many cooking recipes.", 13, 10,
            createIngredientsMap(new Crop(CropType.UNMILLED_RICE), 1), 100),
    COFFEE("Coffee", "It smells delicious. This is sure to give you a boost.", 75, 2,
            createIngredientsMap(new Crop(CropType.COFFEE_BEAN), 5), 150),
    ...
    MEAD("Mead", "A fermented beverage made from honey. Drink in moderation.", 100, 10,
            createIngredientsMap(new ProcessedItem(ProcessedItemType.HONEY), 1), 300),
    PALE_ALE("Pale Ale", "Drink in moderation.", 50, 72,
            createIngredientsMap(new Crop(CropType.HOPS), 1), 300),
    ...
    RAISINS("Raisins", "It's said to be the Junimos' favorite food.", 125, 24,
            createIngredientsMap(new Crop(CropType.GRAPE), 5), 600),
    COAL("Coal", "Turns 10 pieces of wood into one piece of coal.", 0, 1,
            createIngredientsMap(new Material(MaterialType.WOOD), 10), 50),
    CLOTH("Cloth", "A bolt of fine wool cloth.", 0, 4,
            createIngredientsMap(new AnimalProduct(AnimalProductType.WOOL), 1), 470),
    MAYONNAISE("Mayonnaise", "It looks spreadable.", 50, 3,
            createIngredientsMap(new AnimalProduct(AnimalProductType.CHICKEN_EGG), 1), 190),
    LARGE_MAYONNAISE("Large Mayonnaise", "It looks spreadable.", 50, 3,
            createIngredientsMap(new AnimalProduct(AnimalProductType.CHICKEN_EGG), 1), 237),
    DUCK_MAYONNAISE("Duck Mayonnaise", "It's a rich, yellow mayonnaise.", 75, 3,
            createIngredientsMap(new AnimalProduct(AnimalProductType.DUCK_EGG), 1), 375),
    DINOSAUR_MAYONNAISE("Dinosaur Mayonnaise",
            ...
            createIngredientsMap(new AnimalProduct(AnimalProductType.DINOSAUR_EGG), 1), 800),
    TRUFFLE_OIL("Truffle Oil", "A gourmet cooking ingredient.", 38, 6,
            createIngredientsMap(new AnimalProduct(AnimalProductType.TRUFFLE), 1), 1065),
    OIL("Oil", "All-purpose cooking oil.", 13, 6,
            createIngredientsMap(new Crop(CropType.CORN), 1), 100),
    ...
    ProcessedItemType(String name, String description, int energy, int processingTime, HashMap<Item, Integer> ingredients, double sellPrice) {
        this.name = name;
